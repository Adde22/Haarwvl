...
def HaarWavelet(array):
    ...
    for i in range(haararray.shape[0]):
        haararray[i//2,i]=sqrt(2)/2
        haararray[-i//2-1,-i-1]=-sqrt(2)/2
    for i in range(haararray.shape[0]//2):
        haararray[-i-1,-2*i-1]=sqrt(2)/2
    b=dot(haararray,array)
    haararray1=np.zeros((array.shape[1],array.shape[1]))
    for i in range(haararray1.shape[1]):
        haararray1[i//2,i]=sqrt(2)/2
        haararray1[-i//2-1,-i-1]=-sqrt(2)/2
    for i in range(haararray1.shape[1]//2):
        haararray1[-i-1,-2*i-1]=sqrt(2)/2
    t=np.transpose(haararray1)
    e=dot(b,t)
    sm.imsave('AAA.jpg',e)
    aa=e[:e.shape[0]//2,:e.shape[1]//2]
    sm.imsave('aa.jpg',aa)
    ab=e[:e.shape[0]//2,e.shape[1]//2:]
    sm.imsave('ab.jpg',ab)
    ac=e[e.shape[0]//2:,:e.shape[1]//2] 
    sm.imsave('ac.jpg',ac)
    ad=e[e.shape[0]//2:,e.shape[1]//2:]  
    sm.imsave('ad.jpg',ad)
    return e
...
def Nomatrixcompress(array):
    ...
    Non=Non[:Non.shape[0]//2,:Non.shape[1]//2]
